...
public class ChartFactory {
	...
	/**Retorna as seguintes estatisticas para as atividades:
	 * 
	 * 
	 *     Map-Key               Map-Value
	 * 	"totalPassos"	    (double) totalPassos
	 * 	"mediaPassos"  		(double) (totalPassos / totalAtividades)
	 * 	"totalDistancia" 	totalDistancia
	 * 	"mediaDistancia" 	totalDistancia / totalAtividades
	 * 	"totalCalorias"  	totalColoriasPerdidas
	 * 	"mediaCalorias"  	totalColoriasPerdidas / totalAtividades
	 * 
	 * @param atividades
	 * @return
	 */
	public static Map<String, Double> createStatistics(List<Atividade> atividades){
		long totalPassos = 0, totalAtividades = atividades.size();
		double totalDistancia = 0, totalColoriasPerdidas = 0;

		for(Atividade a : atividades) {
			totalPassos += a.getPassos();
			totalDistancia += a.getDistancia();
			totalColoriasPerdidas += a.getCalorias();
		}

		Map<String,Double> statistics = new HashMap<>();
		if(atividades.size() > 0) {
			statistics.put("totalPassos", (double) totalPassos);
			statistics.put("mediaPassos", (double) (totalPassos / totalAtividades));

			statistics.put("totalDistancia", totalDistancia);
			statistics.put("mediaDistancia", totalDistancia / totalAtividades);

			statistics.put("totalCalorias", totalColoriasPerdidas);
			statistics.put("mediaCalorias", totalColoriasPerdidas / totalAtividades);
		}
